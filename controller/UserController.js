...
module.exports = {
    ...
    async addDefultSms(req, res) {
        let { sms_text, notif_text , link_text } = req.body
        if (sms_text, notif_text) {
            try {
                let sms_notif;
                if(link_text){
                    sms_notif = await addSmsData({ sms: sms_text, notif: notif_text, link: link_text })
                } else {
                    sms_notif = await addSmsData({ sms: sms_text, notif: notif_text, link: null })
                }
    ...
    async getLandingData(req, res) {
        ...
        if (poshtibani , update , buy){
            ...
            /*            res.status(200).json({
            buyLink : `${buy[0].address},${buy[0].name}`, 
            updateLink: `${update[0].address},${update[0].name}`,
            poshtibaniLink: `${poshtibani[0].address},${poshtibani[0].name}`
            });*/
        } else {
            res.status(400).json({})
        }
